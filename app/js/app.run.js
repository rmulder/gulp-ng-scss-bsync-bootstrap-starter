(function () {
  ...
  initializeAppNameSpace.$inject = ['$window', '$rootScope', 'Utilities'];
  function initializeAppNameSpace() {
  ...
  function registerStateTracker($rootScope, $state) {
    ...
    $rootScope.$on('$stateChangeSuccess', function () {
      ...
      console.log('$stateChangeSuccess - $state: ', $state);
      $rootScope.headerClass = params.headerClass || '';
  ...
  function registerGlobalEvents($window, $document, $rootScope, $timeout, util) {
    var windowEl = angular.element($window), documentEl = angular.element($document);
    ...
    var lastScrollPosition = 0; documentEl.bind('scroll', util._.throttle(function () {
      ...
      $timeout(function () {
        $rootScope.$broadcast('app:window:scroll', data);
      });
